...
def pointcloud_to_rangeimage(pointcloud, lidar_angle_xy_range, max_lidar_angle_z, min_lidar_angle_z, range_x, range_y):
    ...
    for i in range(length):
        ...
        y_rangeimage = math.floor(degree_in_xy / lidar_angle_xy_range * range_y)
        degree_in_z = calc_theta_degree_in_z(point_world)
        if (degree_in_z >= max_lidar_angle_z):
            print("Theta > %f, degree_in_z = %f" % (max_lidar_angle_z, degree_in_z))
            degree_in_z = max_lidar_angle_z - 0.001
        if (degree_in_z <= min_lidar_angle_z):
            print("Theta < %f, degree_in_z = %f" % (min_lidar_angle_z, degree_in_z))
            degree_in_z = min_lidar_angle_z + 0.001
        # x_rangeimage = math.floor((max_lidar_angle_z - degree_in_z) / (max_lidar_angle_z - min_lidar_angle_z) * range_x)
        x_rangeimage = math.floor(
            (degree_in_z - min_lidar_angle_z) / (max_lidar_angle_z - min_lidar_angle_z) * range_x)
